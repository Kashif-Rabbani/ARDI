package com.genesis.xml.tr.com.srdc.ontmalizer;
...
import com.genesis.xml.tr.com.srdc.ontmalizer.helper.AnnotationFactory;
import com.genesis.xml.tr.com.srdc.ontmalizer.helper.Constants;
import com.genesis.xml.tr.com.srdc.ontmalizer.helper.NamingUtil;
import com.genesis.xml.tr.com.srdc.ontmalizer.helper.SimpleTypeRestriction;
import com.genesis.xml.tr.com.srdc.ontmalizer.helper.URLResolver;
import com.genesis.xml.tr.com.srdc.ontmalizer.helper.XSDUtil;
...
public class XSD2OWLMapper {
    ...
    private String mainURI = "http://www.BDIOntology.com/source/";
    
    // XML Source Mappings
    /*private SourceMappingStruct sourceMappings;
    private String xmlFilePath;
    private Document xmlDomDoc;
    HashMap<String, Integer> createdOwlElems;*/
    HashMap<String, String> dtpropTypes;
...
/*        try {
            ...
            if (uri.isAbsolute()) {
                ...
                mainURI = "http://www.BDIOntology.com/source/";
            }
    ...
    public void convertXSD2OWL() {
        ...
        while (simpleTypes.hasNext()) {
            XSSimpleType simpleTyp = simpleTypes.next();
            System.out.println("SimpleTypes: "+simpleTyp);
            convertSimpleType((XSSimpleType) simpleTyp, null);
            
        }

        Iterator<XSComplexType> complexTypes = schema.iterateComplexTypes();
        while (complexTypes.hasNext()) {
            XSComplexType complexTyp = complexTypes.next();
            System.out.println("SimpleTypes: "+complexTyp);
            convertComplexType((XSComplexType) complexTyp, null);
        }
        ...
        while (elements.hasNext()) {
            ...
            convertElement((XSElementDecl) element, null);
        }

        Iterator<XSModelGroupDecl> groups = schema.iterateModelGroupDecls();
        while (groups.hasNext()) {
            XSModelGroupDecl gr = groups.next();
            System.out.println("Groups: "+gr.toString());
            convertModelGroupDecl(gr);
        }

        Iterator<XSAttGroupDecl> attGroups = schema.iterateAttGroupDecls();
        while (attGroups.hasNext()) {
            XSAttGroupDecl attG = attGroups.next();
            System.out.println("attGroups: "+attG.toString());
            convertAttributeGroup((XSAttGroupDecl) attG);
        }
    ...
    private OntClass convertComplexType(XSComplexType complex, String parentURI) {
        ...
        if (complex.isGlobal()) {
            ...
            //** Here are complex types for test
    ...
    private void convertAttribute(XSAttributeUse attributeUse, OntClass complexClass) {
        ...
        DatatypeProperty prop = ontology.createDatatypeProperty(mainURI + "#" + NamingUtil.createPropertyName(dtpprefix, attribute.getName()));
        ...
        if (NS.equals(XSDDatatype.XSD)) {
            prop.addDomain(complexClass);
            prop.addRange(XSDUtil.getXSDResource(attribute.getType().getName()));
        } else if (ontology.getOntResource(URI + Constants.DATATYPE_SUFFIX) != null) {
            prop.addDomain(complexClass);
            prop.addRange(ontology.getOntResource(URI + Constants.DATATYPE_SUFFIX));
        } else if (ontology.getOntResource(URI) != null) {
            prop.addDomain(complexClass);
            prop.addRange(ontology.getOntResource(URI));
        }
    }
    ...
    private void convertGroup(XSModelGroup group, OntClass parent) {
        ...
        for (XSParticle p : particles) {
            ...
            if (term.isElementDecl()) {
                ...
                Property prop = null;
                Resource propRange = null;
                Resource cardinalitySide = null;
                if (element.getType().isSimpleType()) {
                    ...
                    this.dtpropTypes.put(element.getName(), element.getType().getName());

                    if (element.getType().getTargetNamespace().equals(XSDDatatype.XSD)) {
                        if (element.getType().getName().equals("anyType")) {
                            propRange = OWL.Thing;
                        } else {
                            propRange = XSDUtil.getXSDResource(element.getType().getName());
                        }
                    } else {
                        propRange = ontology.getResource(getURI(element.getType()) + Constants.DATATYPE_SUFFIX);
                        convertSimpleType(element.getType().asSimpleType(), getURI(element.getType()));
                    }
                    ((DatatypeProperty)prop).addDomain(parent);
                    ((DatatypeProperty)prop).addRange(propRange);
                    cardinalitySide = parent;
                    ...
                    if (element.getType().getTargetNamespace().equals(XSDDatatype.XSD)) {
                        ...
                        if (element.getType().isGlobal()) {
                            propRange = ontology.createResource(getURI(element.getType()));
                    ...
                    ((ObjectProperty)prop).addDomain(parent);
                    ((ObjectProperty)prop).addRange(propRange); 
                    cardinalitySide = propRange;
                    ontology.createCardinalityRestriction(null, prop, 1).addSubClass(parent);
                }
                ...
                if (maxOccurs == 1) {
                    if (minOccurs == 1) {
                        ontology.createCardinalityRestriction(null, prop, 1).addSubClass(cardinalitySide);
                    } else // minOccurs can be 0 in this case, logically
                    {
                        ontology.createMaxCardinalityRestriction(null, prop, 1).addSubClass(cardinalitySide);
                    }
                } else {
                    ontology.createMinCardinalityRestriction(null, prop, minOccurs).addSubClass(cardinalitySide);
                    if (maxOccurs == -1) {
                        ontology.createMaxCardinalityRestriction(null, prop, Integer.MAX_VALUE).addSubClass(cardinalitySide);
                    } else {
                        ontology.createMaxCardinalityRestriction(null, prop, maxOccurs).addSubClass(cardinalitySide);
                    }
                }
    ...
    /*public void writeSourceMappings(String filePath) throws ParserConfigurationException, TransformerException {
        SourceMappingXMLWriter writer = new SourceMappingXMLWriter(this.sourceMappings, filePath);
        writer.writeXML();
    }*/
    ...
    public String getDatatypePropertyType(String dtprop) {
        return this.dtpropTypes.get(dtprop);
    }
